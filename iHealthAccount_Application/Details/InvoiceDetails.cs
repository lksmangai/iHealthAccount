...
namespace iHealthAccount.Application.Details
{
    public partial class InvoiceDetails : Form
    {
        ...
        {
            dataGridViewInv.DataSource = homeInvReport.HomeInvoiceDetails();
            dataGridViewInv.Columns[0].Visible = false;
        }   
        ...
        private void AddCustomers(string custCode, string firstName, string middleName, string lastName, int entryBy)
        {
            ...
            if (true)//inv.AddNewInvoice(custCode, firstName, middleName, lastName, entryBy))
            {
        ...
        private void UpadteCustomers(int custId, string custCode, string firstName, string middleName, string lastName, int entryBy)
        {
            ...
            if (true)//inv.ModifyInvoices(custId, custCode, firstName, middleName, lastName, entryBy))
            {
        ...
        private void GetDataForUpdateAndDelete(int expenseID)
        {
            ...
            dt = inv.GetDisplayData(expenseID);
            if (dt.Rows.Count == 0)
                return;
        ...
        {/*
            ...
            if (dataGridViewInv.Rows.Count > 0)
            {
        ...
        {
            if (invByUserID != SessionParameters.UserID)
            {
                buttonUpdate.Enabled = false;
                buttonDelete.Enabled = false;
        ...
        */}
        ...
        private void buttonUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (SessionParameters.UserID != invByUserID)
            {
                labelMessage.Text = MessageManager.GetMessage("4"); ;
                return;
            }
            ...
            /*
            int itemId = DataFormat.GetInteger(dataGridViewInv.SelectedRows[0].Cells[0].Value.ToString());
            string message = string.Empty;
            string custCode = textBoxCustCode.Text.Trim();
            string firstName = textBoxFirstName.Text.Trim();
            string lastName = textBoxLastName.Text.Trim();
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageManager.DisplayMessage("15", new string[] { iHealthAccount.Configurations.ApplicationConfiguration.ExpenseCCY, dataGridViewInv.SelectedRows[0].Cells["ProcedureAmount"].Value.ToString(), dataGridViewInv.SelectedRows[0].Cells["Item"].Value.ToString() }, MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                //Logger.WriteTrace("CustomerMaster [Delete Expense]", "User : " + SessionParameters.UserName + Environment.NewLine + "Expense Id : " + procId);
                inv.DeleteInvoices(invId);
                DisplayDataInGrid();
                ClearFunction();
            }            
        }
        ...
        {
            errorProvider1.Clear();
            dataGridViewInv.Text = String.Empty;
            object value = dataGridViewInv.SelectedRows[0].Cells[0].Value;
            string strValue = (value == null) ? "1" : value.ToString();

            invId = DataFormat.GetInteger(strValue);
            invByUserID = DataFormat.GetInteger(dataGridViewInv.SelectedRows[0].Cells["UserId"].Value.ToString());
            GetDataForUpdateAndDelete(invId);
        ...
        {
            Details.ReciptDetails recipt = new ReciptDetails();
            recipt.Show();
        }
