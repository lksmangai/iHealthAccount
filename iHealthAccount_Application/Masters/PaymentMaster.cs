...
{
    public partial class PaymentMaster : Form
    {
        ...
        {
            dataGridViewPay.DataSource = homePayReport.HomeCustomerDetails();
            dataGridViewPay.Columns[0].Visible = false;
        }
        ...
        private void AddPayment(string paymentMode, string paymentDesc, int entryBy)
        {
            errorProvider1.Clear();
            labelMessage.Text = string.Empty;

            if (pay.AddNewPayment(paymentMode, paymentDesc, entryBy))
            {
        ...
        private void UpadtePayment(int paymentId, string paymentMode, string paymentDesc, int entryBy)
        {
            errorProvider1.Clear();
            labelMessage.Text = string.Empty;

            if (pay.ModifyPayment(paymentId, paymentMode, paymentDesc, entryBy))
            {
        ...
        private void GetDataForUpdateAndDelete(int paymentID)
        {
            DataTable dt = new DataTable();
            dt = pay.GetDisplayData(paymentID);
            if (dt.Rows.Count == 0)
                return;

            textBoxPayMode.Text = DataFormat.GetString(dt.Rows[0][1]);
            textBoxPayMode.Enabled = false;
            textBoxPayDesc.Text = DataFormat.GetString(dt.Rows[0][2]);
        }

        private void ClearFunction()
        {
            errorProvider1.Clear();
            textBoxPayMode.Text = string.Empty;
            textBoxPayDesc.Text = string.Empty;
            textBoxPayDesc.Text = string.Empty;
            textBoxPayMode.Enabled = true;
            ...
            if (dataGridViewPay.Rows.Count > 0)
            {
        ...
        {
            if (payByUserID != SessionParameters.UserID)
            {
                buttonUpdate.Enabled = false;
                buttonDelete.Enabled = false;
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            string description = textBoxPayDesc.Text.Trim();
            string message = string.Empty;
            string mode = textBoxPayMode.Text.Trim();

            if (mode == string.Empty)
            {
                message = MessageManager.GetMessage("5");
                errorProvider1.SetError(textBoxPayMode, message);
            ...
            if (description == string.Empty)
            {
                ...
                errorProvider1.SetError(textBoxPayDesc, message);
                labelMessage.Text = message;
                return;
            }

            //Logger.WriteTrace("ProcedureMaster [Add new Procedure]", "User : " + SessionParameters.UserName + Environment.NewLine + "Item Id: " + itemId.ToString() + Environment.NewLine + "Description : " + textBoxProcName.Text.Trim() + Environment.NewLine + "Price : " + textBoxPayDesc.Text.Trim());
            AddPayment(mode, description, SessionParameters.UserID);
        }
        ...
        private void buttonUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (SessionParameters.UserID != payByUserID)
            {
                labelMessage.Text = MessageManager.GetMessage("4"); ;
                return;
            }
            ...
            int itemId = DataFormat.GetInteger(dataGridViewPay.SelectedRows[0].Cells[0].Value.ToString());
            string mode = textBoxPayMode.Text.Trim();
            string description = textBoxPayDesc.Text.Trim();
            ...
            //Logger.WriteTrace("ProcedureMaster [Update Expense]", "User : " + SessionParameters.UserName + Environment.NewLine + "Expense Id : " + procId + Environment.NewLine + "Description : " + textBoxProcName.Text.Trim() + Environment.NewLine + "Price : " + textBoxPayDesc.Text.Trim());
            UpadtePayment(itemId, mode, description, SessionParameters.UserID);
            ClearFunction();
        }
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageManager.DisplayMessage("15", new string[] { iHealthAccount.Configurations.ApplicationConfiguration.ExpenseCCY, dataGridViewPay.SelectedRows[0].Cells["ProcedureAmount"].Value.ToString(), dataGridViewPay.SelectedRows[0].Cells["Item"].Value.ToString() }, MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                //Logger.WriteTrace("ProcedureMaster [Delete Expense]", "User : " + SessionParameters.UserName + Environment.NewLine + "Expense Id : " + procId);
                pay.DeletePayment(payId);
                DisplayDataInGrid();
                ClearFunction();
            }
        }
        ...
        {
            errorProvider1.Clear();
            dataGridViewPay.Text = String.Empty;
            object value = dataGridViewPay.SelectedRows[0].Cells[0].Value;
            string strValue = (value == null) ? "1" : value.ToString();

            payId = DataFormat.GetInteger(strValue);
            payByUserID = DataFormat.GetInteger(dataGridViewPay.SelectedRows[0].Cells["UserId"].Value.ToString());
            GetDataForUpdateAndDelete(payId);
